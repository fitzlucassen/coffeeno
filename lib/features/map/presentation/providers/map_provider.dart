...
final mapScopeProvider = NotifierProvider<MapScopeNotifier, MapScope>(
  MapScopeNotifier.new,
);
...
final originStatsProvider = StreamProvider<List<OriginStats>>((ref) {
  ...
  // Signed-out users (uid == null) fall through to the global aggregate even
  // in `mine` scope.
  final uid = scope == MapScope.mine
      ? ref.watch(authStateProvider).value?.uid
      : null;
  return repository.getOriginStats(uid: uid);
});

/// Streams coffees for a specific origin country.
final coffeesByOriginProvider =
    StreamProvider.family<List<LeaderboardEntry>, String>((ref, country) {
      final repository = ref.watch(mapRepositoryProvider);
