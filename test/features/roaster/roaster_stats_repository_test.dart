...
void main() {
  ...
  group('getRecentTastingsForRoaster', () {
    test(
      'returns tastings for this roaster\'s coffees, newest first',
      () async {
        ...
        await _seedTasting(
          firestore,
          _tasting(coffeeId: c2, createdAt: DateTime(2026, 5, 5)),
        );
        await _seedTasting(
          firestore,
          _tasting(coffeeId: c3, createdAt: DateTime(2026, 5, 10)),
        );
  ...
  group('getTimeseriesForRoaster', () {
    ...
    test('tastings are attributed to the correct bucket', () async {
      ...
      final fifthMay = points.firstWhere(
        (p) => p.date.year == 2026 && p.date.month == 5 && p.date.day == 5,
      );
  ...
  group('buildRoasterExportCsv', () {
    test('produces a CSV with a header row and one row per tasting', () async {
      final c1 = await _seedCoffee(firestore, _coffee(id: 'x'));
      await _seedTasting(
        firestore,
        _tasting(coffeeId: c1, createdAt: DateTime(2026, 5, 1)),
    ...
    test('returns just the header when the roaster has no coffees', () async {
